...
def render_dashboard() -> None:
    ...
    tab_a, tab_b, tab_c, tab_d, tab_e = st.tabs(
        ["Trends & comparison", "Distributions & frontier", "Flow & utilization", "Explainability & drill-down", "Event Logs History"]
    )
    ...
    with tab_e:
        st.markdown("**Historical Routing Event Logs**")
        st.caption("A complete table of routing requests and telemetry metadata for analysis.")
        ...
        if "ts" in display_df.columns:
            display_df["ts"] = display_df["ts"].dt.strftime("%Y-%m-%d %H:%M:%S")
            rename_map["ts"] = "Time (UTC)"
        if "request_id" in display_df.columns:
            rename_map["request_id"] = "Request ID"
        if "query_type" in display_df.columns:
            rename_map["query_type"] = "Query Type"
        if "complexity_level" in display_df.columns:
            rename_map["complexity_level"] = "Complexity"
        if "routed_model" in display_df.columns:
            rename_map["routed_model"] = "Routed Model"
        if "routed_cost_usd" in display_df.columns:
            display_df["routed_cost_usd"] = display_df["routed_cost_usd"].map(lambda x: f"${x:.6f}" if pd.notnull(x) else "")
            rename_map["routed_cost_usd"] = "Cost (USD)"
        if "routed_latency_ms" in display_df.columns:
            display_df["routed_latency_ms"] = display_df["routed_latency_ms"].map(lambda x: f"{x:.0f} ms" if pd.notnull(x) else "")
            rename_map["routed_latency_ms"] = "Latency"
        if "est_savings_vs_brain_usd" in display_df.columns:
            display_df["est_savings_vs_brain_usd"] = display_df["est_savings_vs_brain_usd"].map(lambda x: f"${x:.6f}" if pd.notnull(x) else "")
            rename_map["est_savings_vs_brain_usd"] = "Est. Savings"
            
        # Select order of columns
        cols_in_order = [
            "Time (UTC)", "Request ID", "Complexity", "Query Type", 
            "Routed Model", "Latency", "Cost (USD)", "Est. Savings"
        ]
        
        display_df = display_df.rename(columns=rename_map)
        
        # Only keep columns we explicitly renamed (so we don't show raw json blocks), if they exist
        display_cols = [c for c in cols_in_order if c in display_df.columns]
        
        # Optional: add rationale or other raw columns at the end if you want them visible
